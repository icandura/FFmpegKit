...
namespace FFmpegKit
{
    public partial class ConcatForm : Form
    {
        ...
        public ConcatForm()
        {
            ...
            // 参数联动
            radVideo.CheckedChanged += RadType_CheckedChanged;
            radAudio.CheckedChanged += RadType_CheckedChanged;

            chkFastMode.CheckedChanged += (s, e) => UpdateControlsVisibility();
            nudHeight.ValueChanged += (s, e) => UpdateWidthDisplay();
            cmbAspect.SelectedIndexChanged += (s, e) => UpdateWidthDisplay();
        }

        private void ConcatForm_Load(object sender, EventArgs e)
        {
        ...
        {
            bool isVideo = radVideo.Checked;
            bool isFastMode = chkFastMode.Checked;
            bool enableParams = isVideo && !isFastMode;     //快速模式下也不能设置参数

            lblHeight.Enabled = enableParams;
            nudHeight.Enabled = enableParams;
            lblAspect.Enabled = enableParams;
            cmbAspect.Enabled = enableParams;
            lblWidthAuto.Enabled = enableParams;
            lblFPS.Enabled = enableParams;
            cmbFPS.Enabled = enableParams;

            // 配置是否允许使用GPU加速
            if (ConfigManager.DefaultGPU == "none" || isFastMode)
            {
        ...
        private void BtnStart_Click(object sender, EventArgs e)
        {
            ...
            bool isVideoOutput = radVideo.Checked;
            int height = (int)nudHeight.Value;
            string ratio = cmbAspect.Text;
